...
then
	...
	# Download
	wget --no-check-certificate -O /tmp/apps-scripts/regataos-repo.tar.xz https://master.dl.sourceforge.net/project/regataos/repos/regataos-repo.tar.xz 2>&1 | sudo -H env DISPLAY=:0 zenity --progress --pulsate --width 350 --text "Baixando arquivos. Espere um momento..." --title "Atualizando os repositórios de software" --auto-close --auto-kill --no-cancel

	# Extract files
	sudo tar xf /tmp/apps-scripts/regataos-repo.tar.xz -C /
